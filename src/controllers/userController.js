...
exports.googlelogin = async (req, res, next) => {
  ...
  try{
    ...
    res.status(412).send({ "isSuccess": false, "code": 412, "message": "[ERROR] Google AccessToken error" });
    return;
  }
  ...
  db.query("SELECT id, name, userToken FROM user WHERE snsId='" +user_info.sub + "' AND sns='google';", function(err, result, field){
    if (err) throw err;
