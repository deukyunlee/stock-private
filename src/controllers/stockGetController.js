...
module.exports.stock_company_specific_get = (req, res, next) => {
  ...
  const sql = `select c.symbol, d.close,if(d.change_percent>0,concat("+",ROUND(d.change_percent,1)),ROUND(d.change_percent,1)) as change_percent, if(d.change_value>0,concat("+",d.change_value*1269),d.change_value*1269) as change_value, c.name_en, c.name_kr, c.desc_en, c.desc_kr, img, shareout from daily as d inner join company_info as c on d.symbol = c.symbol where d.symbol = ? and change_percent is not null group by symbol;`;
  db.query(sql, symbol, function (err, rows, fields) {
    res.json(rows);
  });
};
