...
module.exports.stock_company_specific_get = (req, res, next) => {
  const symbol = req.params.symbol;
  const sql = `select c.symbol, d.close, d.change_percent, d.change_value, c.name_en, c.name_kr, c.desc_en, c.desc_kr, img, shareout from daily as d, company_info as c where d.symbol = c.symbol and d.symbol = ?;`;
  db.query(sql, symbol, function (err, rows, fields) {
    res.json(rows);
  });
};
