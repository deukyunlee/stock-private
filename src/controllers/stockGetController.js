...
module.exports.stock_company_specific_get = (req, res, next) => {
  ...
  const sql = `select c.symbol, d.close, d.change_percent, d.change_value, c.name_en, c.name_kr, c.desc_en, c.desc_kr, img, shareout from daily as d inner join company_info as c on d.symbol = c.symbol where d.symbol = ? and change_percent is not null group by symbol;`;
  db.query(sql, symbol, function (err, rows, fields) {
    res.json(rows);
  });
};
