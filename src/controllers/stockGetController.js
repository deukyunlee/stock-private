...
module.exports.stock_daily_monthly_get = (req, res, next) => {
  ...
  const sql1 = `select symbol, DATE_FORMAT(date,'%Y-%m-%d') as date, open, high, low, close, volume from daily where symbol = "${symbol}" and date between date_add(now(), interval-1 month) and now() order by date;`;
...
module.exports.stock_company_specific_get = (req, res, next) => {
  ...
  db.query(sql1, symbol, function (err, rows, fields) {
    ...
    const sql = `select c.symbol, d.close,if(d.change_percent>0,concat("+",ROUND(d.change_percent,2)),ROUND(d.change_percent,2)) as change_percent, if(d.change_value>0,concat("+",ROUND(d.change_value*1269,2)),ROUND(d.change_value*1269,2)) as change_value, c.name_en, c.name_kr, c.desc_en, c.desc_kr, img, shareout from daily as d inner join company_info as c on d.symbol = c.symbol where c.symbol = ? and change_percent is not null order by d.date desc limit 1;`;
    db.query(sql, symbol, function (err, rows, fields) {
      res.json(rows);
    });
  });
