...
exports.verifyToken = async (req, res, next) => {
  ...
  //{ userId } = req.body;
  if(!userToken) {
    return res.status(412).send({ "isSuccess": false, "code": 412, "message": "user token EMPTY" })
  }
  ...
  try {
    ...
    if (error.name === 'TokenExpiredError') {
      return res.status(412).json({ "isSuccess": false, "code": 412, "message": "usertoken만료" })
    }
    // 토큰의 비밀키가 일치하지 않는 경우
    return res.status(412).json({ "isSuccess": false, "code": 412, "message": "유효하지않은usertoken" })
  }
  ...
  db.query("SELECT id, userToken FROM user WHERE id='" +decoded_user_info.id+ "' AND sns='"+decoded_user_info.sns+"';", function(err, result, field){
    if (result.length) { //있으면 userId, userToken 전송
      ...
      return res.status(412).json({ "isSuccess": false, "code": 412, "message": "INVALID usertoken" })
    }
  })
